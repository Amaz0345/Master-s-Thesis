#### Ananya Mazumder MSc. Development Economics Uni Göttingen ####
...
View(data_2019)
##adding the variables for control list 
#women's age at first cohabitation: v511 
#women's height: v438
#women's weight: v437
#men's age at first marriage: mv511 
#men's height:
#men's weight: mv005
#wealth indices scores: mv190 mv191 mv190a mv191a
#more wealth index scores (State): sm190s sm191s sm190us sm191us sm190rs sm191rs
IAIR7EFL_select <- IAIR7EFL %>%
  select(caseid, v511, v438, v437)

data_joined <- data_2019 %>%
  left_join(
    IAIR7EFL_select,
    by = "caseid"
  )
IAMR7EFL_select <- IAMR7EFL %>%
  select(mcaseid, sm190s, sm191s)
...
write_dta(data_joined, "2019_final.dta")
...
data_selected_women <- data_selected_women %>% select(-eligibility_status) ##dropping columns by name 

table(data_selected_women$treated_district)

## Construction of individual level controls ##
#v012 = woman's age,v133= woman's edu in years, v43= woman's height (missing from dataset), v714= respondent currently working, v715= husband's years of education, d113= husband/partner drinks alcohol  
## Construction of household level controls ##
#v152= age of hh head,v151= sex of hh head, v136= number of hh members, v137 = number of children 5 years and younger,v119-v125 = asset index, v191= wealth index, v025= type of place of residence, v131= belongs to SC or not,  
...
##exclude observations missing any results on controls/outcomes 
...
desc_table_3 %>% as_gt() ##fix order of specified controls, the output is weird 

## Testing Parallel Trends Assumption ##
## DiD ## 
## Decision Making models ##
# the Fixed Effects Difference-in-Differences model
...
# Combine the list of controls into a single string separated by '+'
## heterogeneities in effects ## 
# look at effects by caste, religion, "ethnicity" 
## Placebo Tests ##
